...
- gates bloquants en CI : lint, format:check, typecheck, build, test — et le job `image`
  (construction d'`api/Dockerfile`, code non inscriptible par l'utilisateur d'exécution, scan Trivy
  rouge sur une vulnérabilité critique corrigeable — politique verrouillée par #45 —,
  `docker compose config` du compose de production avec `.env.example`, `caddy validate` du
  Caddyfile par l'image épinglée)
