...
jeton **public `pk.`** aux quatre portées de lecture `styles:read`, `styles:tiles`,
`fonts:read` et `datasets:read`, nommées une à une — pas « les portées publiques par défaut », que
la console coche plus largement (`vision:read`, à décocher). **Ne demander ni `styles:list` ni
`styles:write`** : ce sont des
